...
FLAGS="-pedantic -D_GNU_SOURCE -std=gnu11 -Werror -Wfloat-conversion -Wimplicit-fallthrough -Wextra -g -lm -ldl -pthread"
if [ "${1}" = "release" ]; then
	FLAGS="$FLAGS -O2"
fi
...
if [ "${1}" = "release" ]; then
	...
	cd build
	tar -cvzf "hcc-0.0.3-linux.tar.gz" hcc ../libc ../libhmaths ../libhccintrinsics ../interop ../samples ../playground ../docs ../README.md ../LICENSE
fi
